...
export async function POST(request: NextRequest) {
  try {
    ...
    // 🔥 ДЛЯ TON: разрешаем демо-режим, но создаём пользователя
    // В продакшене лучше тоже требовать регистрацию, но для гибкости оставляем
    let user = await prisma.user.findUnique({
      where: { id: userId },
    })

    // Если пользователь не найден — создаём (только для TON)
    if (!user) {
      // 🔥 В продакшене можно запретить создание для demo
      if (userId === 'demo') {
        return NextResponse.json(
          {
            error:
              'Демо-режим: покупка за TON недоступна. Пожалуйста, войдите в аккаунт.',
          },
          { status: 403 },
        )
      }

      // Создаём пользователя автоматически (для реальных пользователей)
      ...
      console.log('✅ User created for TON purchase:', userId)
    }
